...
def xyz_lines_from_file(filename):
    with open(filename, "r") as file:
        data = file.readlines()
    return data

def xyz_lines_from_string(xyz_str):
    return xyz_str.splitlines()

def read_xyz_file(filename, look_for_charge=True):
    return read_xyz_lines(xyz_lines_from_file(filename), look_for_charge)

def read_xyz_string(xyz_str, look_for_charge=True):
    return read_xyz_lines(xyz_lines_from_string(xyz_str), look_for_charge)

def read_xyz_lines(xyz_lines, look_for_charge=True):
    """
    """
    ...
    for line_number, line in enumerate(xyz_lines):
        if line_number == 0:
            num_atoms = int(line)
        elif line_number == 1:
            title = line
...
## Experimental Package Functions

def xyz_string_to_smiles(xyz_str):
    atoms, charge, xyz_coordinates = read_xyz_string(xyz_str)
...
# """
#
# print(xyz_string_to_smiles(xstr))
